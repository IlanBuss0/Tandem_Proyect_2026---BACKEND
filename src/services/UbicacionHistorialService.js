...
export default class UbicacionHistorialService {
  ...
  getByIdAsync = async (id) => { console.log(`UbicacionHistorialService.getByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del historial de ubicación es inválido.'); } return await this.UbicacionHistorialRepository.getByIdAsync(id); };
  ...
  deleteByIdAsync = async (id) => { console.log(`UbicacionHistorialService.deleteByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del historial es inválido.'); } return await this.UbicacionHistorialRepository.deleteByIdAsync(id); };

  validarUbicacionParaCrear = (entity) => {
    if (!entity) throw new Error('El historial de ubicación es obligatorio.');
