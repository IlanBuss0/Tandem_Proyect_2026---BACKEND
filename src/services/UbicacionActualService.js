...
export default class UbicacionActualService {
  ...
  getByIdAsync = async (id) => { console.log(`UbicacionActualService.getByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id de la ubicación actual es inválido.'); } return await this.UbicacionActualRepository.getByIdAsync(id); };
  ...
  createAsync = async (entity) => {
    ...
    if (existente != null) { throw new Error(`Ya existe una ubicación actual para el dispositivo con id ${entity.id_dispositivo}.`); }
    return await this.UbicacionActualRepository.createAsync(entity);
  };

  updateAsync = async (entity) => { console.log(`UbicacionActualService.updateAsync(${JSON.stringify(entity)})`); if (!entity?.id || Number.isNaN(entity.id)) { throw new Error('El id de la ubicación actual es obligatorio para actualizar.'); } const prev = await this.UbicacionActualRepository.getByIdAsync(entity.id); if (prev == null) return 0; return await this.UbicacionActualRepository.updateAsync(entity); };

  deleteByIdAsync = async (id) => { console.log(`UbicacionActualService.deleteByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id de la ubicación actual es inválido.'); } return await this.UbicacionActualRepository.deleteByIdAsync(id); };

  validarUbicacionParaCrear = (entity) => {
    if (!entity) throw new Error('La ubicación actual es obligatoria.');
