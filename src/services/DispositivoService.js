...
export default class DispositivoService {
  ...
  getByIdAsync = async (id) => { console.log(`DispositivoService.getByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del dispositivo es invalido.'); } return await this.DispositivoRepository.getByIdAsync(id); };
  ...
  deleteByIdAsync = async (id) => { console.log(`DispositivoService.deleteByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del dispositivo es invalido.'); } return await this.DispositivoRepository.deleteByIdAsync(id); };
