...
export default class BloqueoUsuarioService {
  ...
  getByIdAsync = async (id) => { console.log(`BloqueoUsuarioService.getByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del bloqueo es inválido.'); } return await this.BloqueoUsuarioRepository.getByIdAsync(id); };
  ...
  deleteByIdAsync = async (id) => { console.log(`BloqueoUsuarioService.deleteByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del bloqueo es inválido.'); } return await this.BloqueoUsuarioRepository.deleteByIdAsync(id); };
  ...
  validarBloqueoParaCrear = (entity) => {
    ...
    if (entity.id_usuario_bloqueador === entity.id_usuario_bloqueado) throw new Error('Un usuario no puede bloquearse a sí mismo.');
