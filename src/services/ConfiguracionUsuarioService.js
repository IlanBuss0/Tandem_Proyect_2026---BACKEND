...
export default class ConfiguracionUsuarioService {
  ...
  updateAsync = async (entity) => { console.log(`ConfiguracionUsuarioService.updateAsync(${JSON.stringify(entity)})`); if (!entity?.id || Number.isNaN(entity.id)) { throw new Error('El id de la configuracion es obligatorio para actualizar.'); } const prev = await this.ConfiguracionUsuarioRepository.getByIdAsync(entity.id); if (prev == null) return 0; return await this.ConfiguracionUsuarioRepository.updateAsync(entity); };
  deleteByIdAsync = async (id) => { console.log(`ConfiguracionUsuarioService.deleteByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id de la configuracion es invalido.'); } return await this.ConfiguracionUsuarioRepository.deleteByIdAsync(id); };
