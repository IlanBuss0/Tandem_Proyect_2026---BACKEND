...
export default class ReporteUsuarioService {
  ...
  getByIdAsync = async (id) => { console.log(`ReporteUsuarioService.getByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del reporte es invalido.'); } return await this.ReporteUsuarioRepository.getByIdAsync(id); };
  ...
  deleteByIdAsync = async (id) => { console.log(`ReporteUsuarioService.deleteByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del reporte es invalido.'); } return await this.ReporteUsuarioRepository.deleteByIdAsync(id); };
