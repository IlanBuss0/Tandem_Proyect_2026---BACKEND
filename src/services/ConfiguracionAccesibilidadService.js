...
export default class ConfiguracionAccesibilidadService {
  ...
  getByIdAsync = async (id) => { console.log(`ConfiguracionAccesibilidadService.getByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id de la configuracion de accesibilidad es invalido.'); } return await this.ConfiguracionAccesibilidadRepository.getByIdAsync(id); };
  ...
  createAsync = async (entity) => {
    ...
    if (existente != null) { throw new Error(`Ya existe la configuracion de accesibilidad '${entity.clave}' para el usuario ${entity.id_usuario}.`); }
    return await this.ConfiguracionAccesibilidadRepository.createAsync(entity);
  };

  updateAsync = async (entity) => { console.log(`ConfiguracionAccesibilidadService.updateAsync(${JSON.stringify(entity)})`); if (!entity?.id || Number.isNaN(entity.id)) { throw new Error('El id de la configuracion de accesibilidad es obligatorio para actualizar.'); } const prev = await this.ConfiguracionAccesibilidadRepository.getByIdAsync(entity.id); if (prev == null) return 0; return await this.ConfiguracionAccesibilidadRepository.updateAsync(entity); };
  deleteByIdAsync = async (id) => { console.log(`ConfiguracionAccesibilidadService.deleteByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id de la configuracion de accesibilidad es invalido.'); } return await this.ConfiguracionAccesibilidadRepository.deleteByIdAsync(id); };

  validarConfigParaCrear = (entity) => {
    if (!entity) throw new Error('La configuracion de accesibilidad es obligatoria.');
