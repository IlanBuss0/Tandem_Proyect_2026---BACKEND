...
export default class ChatService {
  ...
  getByIdAsync = async (id) => { console.log(`ChatService.getByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del chat es inválido.'); } return await this.ChatRepository.getByIdAsync(id); };
  ...
  deleteByIdAsync = async (id) => { console.log(`ChatService.deleteByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del chat es inválido.'); } return await this.ChatRepository.deleteByIdAsync(id); };
