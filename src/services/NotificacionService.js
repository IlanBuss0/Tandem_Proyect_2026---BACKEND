...
export default class NotificacionService {
  ...
  getByIdAsync = async (id) => { console.log(`NotificacionService.getByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id de la notificación es inválido.'); } return await this.NotificacionRepository.getByIdAsync(id); };
  ...
  updateAsync = async (entity) => { console.log(`NotificacionService.updateAsync(${JSON.stringify(entity)})`); if (!entity?.id || Number.isNaN(entity.id)) { throw new Error('El id de la notificación es obligatorio para actualizar.'); } const prev = await this.NotificacionRepository.getByIdAsync(entity.id); if (prev == null) return 0; return await this.NotificacionRepository.updateAsync(entity); };

  deleteByIdAsync = async (id) => { console.log(`NotificacionService.deleteByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id de la notificación es inválido.'); } return await this.NotificacionRepository.deleteByIdAsync(id); };

  validarNotificacionParaCrear = (entity) => {
    if (!entity) throw new Error('La notificación es obligatoria.');
