...
export default class MensajeService {
  ...
  getByIdAsync = async (id) => { console.log(`MensajeService.getByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del mensaje es inválido.'); } return await this.MensajeRepository.getByIdAsync(id); };
  ...
  deleteByIdAsync = async (id) => { console.log(`MensajeService.deleteByIdAsync(${id})`); if (!id || Number.isNaN(id)) { throw new Error('El id del mensaje es inválido.'); } return await this.MensajeRepository.deleteByIdAsync(id); };
