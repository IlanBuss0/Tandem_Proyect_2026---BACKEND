...
export default class PermisoArchivoRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM permisos_archivos ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`PermisoArchivoRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM permisos_archivos WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO permisos_archivos (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE permisos_archivos SET ${setClause} WHERE id = $1`;
