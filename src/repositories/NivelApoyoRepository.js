...
export default class NivelApoyoRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM niveles_apoyos ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`NivelApoyoRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM niveles_apoyos WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO niveles_apoyos (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE niveles_apoyos SET ${setClause} WHERE id = $1`;
