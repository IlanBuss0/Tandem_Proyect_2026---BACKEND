...
export default class TipoChatRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM tipos_chats ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`TipoChatRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM tipos_chats WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO tipos_chats (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE tipos_chats SET ${setClause} WHERE id = $1`;
