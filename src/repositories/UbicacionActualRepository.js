...
export default class UbicacionActualRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT id, id_dispositivo, latitud, longitud, fecha_registro FROM ubicaciones_actuales ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`UbicacionActualRepository.getByIdAsync(${id})`);
    const sql = `SELECT id, id_dispositivo, latitud, longitud, fecha_registro FROM ubicaciones_actuales WHERE id = $1`;
  ...
  getByDispositivoIdAsync = async (idDispositivo) => {
    ...
    const sql = `SELECT id, id_dispositivo, latitud, longitud, fecha_registro FROM ubicaciones_actuales WHERE id_dispositivo = $1`;
    return await BD.queryOne(sql, [idDispositivo]);
  };

  createAsync = async (entity) => {
    console.log(`UbicacionActualRepository.createAsync(${JSON.stringify(entity)})`);
    const sql = `INSERT INTO ubicaciones_actuales (id_dispositivo, latitud, longitud, fecha_registro) VALUES ($1, $2, $3, $4) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE ubicaciones_actuales SET id_dispositivo = $2, latitud = $3, longitud = $4, fecha_registro = $5 WHERE id = $1`;
  ...
  deleteByIdAsync = async (id) => {
    ...
    const sql = `DELETE FROM ubicaciones_actuales WHERE id = $1`;
