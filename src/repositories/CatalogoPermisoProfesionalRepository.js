...
export default class CatalogoPermisoProfesionalRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM catalogo_permisos_profesionales ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`CatalogoPermisoProfesionalRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM catalogo_permisos_profesionales WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO catalogo_permisos_profesionales (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE catalogo_permisos_profesionales SET ${setClause} WHERE id = $1`;
