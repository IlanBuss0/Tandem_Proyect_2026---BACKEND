...
export default class EstadoSuscripcionRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM estados_suscripciones ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`EstadoSuscripcionRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM estados_suscripciones WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO estados_suscripciones (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE estados_suscripciones SET ${setClause} WHERE id = $1`;
