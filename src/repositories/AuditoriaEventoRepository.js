...
export default class AuditoriaEventoRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM auditorias_eventos ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`AuditoriaEventoRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM auditorias_eventos WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO auditorias_eventos (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE auditorias_eventos SET ${setClause} WHERE id = $1`;
