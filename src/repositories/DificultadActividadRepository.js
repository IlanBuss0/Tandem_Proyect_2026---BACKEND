...
export default class DificultadActividadRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM dificultades_actividades ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`DificultadActividadRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM dificultades_actividades WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO dificultades_actividades (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE dificultades_actividades SET ${setClause} WHERE id = $1`;
