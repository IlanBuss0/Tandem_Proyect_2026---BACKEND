...
export default class TipoEventoAuditoriaRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM tipos_eventos_auditorias ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`TipoEventoAuditoriaRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM tipos_eventos_auditorias WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO tipos_eventos_auditorias (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE tipos_eventos_auditorias SET ${setClause} WHERE id = $1`;
