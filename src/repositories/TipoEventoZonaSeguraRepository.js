...
export default class TipoEventoZonaSeguraRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM tipos_eventos_zonas_seguras ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`TipoEventoZonaSeguraRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM tipos_eventos_zonas_seguras WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO tipos_eventos_zonas_seguras (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE tipos_eventos_zonas_seguras SET ${setClause} WHERE id = $1`;
