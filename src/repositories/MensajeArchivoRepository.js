...
export default class MensajeArchivoRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM mensajes_archivos ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`MensajeArchivoRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM mensajes_archivos WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO mensajes_archivos (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE mensajes_archivos SET ${setClause} WHERE id = $1`;
