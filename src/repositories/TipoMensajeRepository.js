...
export default class TipoMensajeRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM tipos_mensajes ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`TipoMensajeRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM tipos_mensajes WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO tipos_mensajes (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE tipos_mensajes SET ${setClause} WHERE id = $1`;
