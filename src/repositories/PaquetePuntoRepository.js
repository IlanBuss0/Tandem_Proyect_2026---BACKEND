...
export default class PaquetePuntoRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM paquetes_puntos ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`PaquetePuntoRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM paquetes_puntos WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO paquetes_puntos (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE paquetes_puntos SET ${setClause} WHERE id = $1`;
