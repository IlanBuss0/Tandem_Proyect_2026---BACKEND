...
export default class RoutineReminderRepository {
  replaceForUserAsync = (userId, entries) => BD.transaction(async client => {
    ...
    for(const e of entries) await client.query(`INSERT INTO recordatorios_programados(id_usuario,source_type,routine_id,item_id,titulo,occurrence_at,offset_minutes,scheduled_at) VALUES($1,'routine',$2,$3,$4,$5,$6,$7) ON CONFLICT(id_usuario,source_type,item_id,occurrence_at,offset_minutes) DO UPDATE SET routine_id=EXCLUDED.routine_id,titulo=EXCLUDED.titulo,scheduled_at=EXCLUDED.scheduled_at,status='pending',processed_at=NULL`,[userId,e.routineId,e.itemId,e.title,e.occurrenceAt,e.offsetMinutes,e.scheduledAt]);
