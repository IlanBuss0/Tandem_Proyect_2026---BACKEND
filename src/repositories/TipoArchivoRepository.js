...
export default class TipoArchivoRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM tipos_archivos ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`TipoArchivoRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM tipos_archivos WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO tipos_archivos (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE tipos_archivos SET ${setClause} WHERE id = $1`;
