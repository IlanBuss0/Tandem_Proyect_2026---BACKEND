...
export default class EntidadAfectadaAuditoriaRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM entidades_afectadas_auditorias ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`EntidadAfectadaAuditoriaRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM entidades_afectadas_auditorias WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO entidades_afectadas_auditorias (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE entidades_afectadas_auditorias SET ${setClause} WHERE id = $1`;
