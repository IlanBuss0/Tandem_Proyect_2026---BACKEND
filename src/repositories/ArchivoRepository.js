...
export default class ArchivoRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM archivos ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`ArchivoRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM archivos WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO archivos (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE archivos SET ${setClause} WHERE id = $1`;
