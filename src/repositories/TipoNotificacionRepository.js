...
export default class TipoNotificacionRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM tipos_notificaciones ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`TipoNotificacionRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM tipos_notificaciones WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO tipos_notificaciones (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE tipos_notificaciones SET ${setClause} WHERE id = $1`;
