...
export default class RolAdministradorRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM roles_administradores ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`RolAdministradorRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM roles_administradores WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO roles_administradores (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE roles_administradores SET ${setClause} WHERE id = $1`;
