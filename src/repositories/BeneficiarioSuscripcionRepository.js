...
export default class BeneficiarioSuscripcionRepository {
  ...
  getAllAsync = async () => {
    ...
    const sql = `SELECT * FROM beneficiarios_suscripciones ORDER BY id DESC`;
    return await BD.query(sql);
  };

  getByIdAsync = async (id) => {
    console.log(`BeneficiarioSuscripcionRepository.getByIdAsync(${id})`);
    const sql = `SELECT * FROM beneficiarios_suscripciones WHERE id = $1`;
  ...
  createAsync = async (entity) => {
    ...
    const sql = `INSERT INTO beneficiarios_suscripciones (${fields}) VALUES (${placeholders}) RETURNING id`;
  ...
  updateAsync = async (entity) => {
    ...
    const sql = `UPDATE beneficiarios_suscripciones SET ${setClause} WHERE id = $1`;
