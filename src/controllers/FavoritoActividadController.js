...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (returnEntity != null) {
      ...
      res.status(StatusCodes.NOT_FOUND).send(`No se encontro el favorito con id: ${id}.`);
    }
  } catch (error) {
    console.log(error);
    res.status(StatusCodes.INTERNAL_SERVER_ERROR).send(`Error: ${error.message}`);
  ...
  try {
    ...
    if (newId > 0) {
      res.status(StatusCodes.CREATED).json({
        message: `Se creo el favorito con id: ${newId}`,
        id: newId,
      });
    } else {
  ...
  try {
    ...
    if (rowsAffected !== 0) {
      res.status(StatusCodes.OK).json({
        message: `Se actualizo el favorito con id: ${id}`,
        rowsAffected,
      });
    } else {
      res.status(StatusCodes.NOT_FOUND).send(`No se encontro el favorito con id: ${id}.`);
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (rowCount !== 0) {
      res.status(StatusCodes.OK).json({
        message: `Se elimino el favorito con id: ${id}`,
