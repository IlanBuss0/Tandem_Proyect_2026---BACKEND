...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (returnEntity != null) {
      ...
      res.status(StatusCodes.NOT_FOUND).send(`No se encontró la actividad asignada con id: ${id}.`);
    }
  } catch (error) {
    console.log(error);
    res.status(StatusCodes.INTERNAL_SERVER_ERROR).send(`Error: ${error.message}`);
  ...
  try {
    ...
    if (newId > 0) {
      res.status(StatusCodes.CREATED).json({
        message: `Se creó la actividad asignada con id: ${newId}`,
        id: newId,
      });
    } else {
  ...
  try {
    ...
    if (rowsAffected !== 0) {
      res.status(StatusCodes.OK).json({
        message: `Se actualizó la actividad asignada con id: ${id}`,
        rowsAffected,
      });
    } else {
      res.status(StatusCodes.NOT_FOUND).send(`No se encontró la actividad asignada con id: ${id}.`);
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (rowCount !== 0) {
      res.status(StatusCodes.OK).json({
        message: `Se eliminó la actividad asignada con id: ${id}`,
