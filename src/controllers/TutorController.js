...
router.get('/:id/pertenecientes', async (req, res, next) => { try { res.status(200).json({ ok: true, data: await TutorService.listPertenecientes(Number(req.params.id)) }); } catch (e) { next(e); } });
